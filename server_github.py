...
if not API_KEY:
    log.warning("GITHUB_TOKEN not set in .env")
...
def api_update_issue(
    ...
    with build_client() as client:
        ...
        if patch_payload:
            url = f"{BASE_URL}/repos/{GITHUB_USERNAME}/{repo}/issues/{issue}"
        ...
        # Optional comment
        if comment:
            url_c = f"{BASE_URL}/repos/{GITHUB_USERNAME}/{repo}/issues/{issue}/comments"
            res_c = client.post(
                url_c,
                headers=get_headers(),
                json={
...
def create_repo(
    ...
    try:
        ...
        if create_readme:
            try:
                r = api_create_readme(name, readme_content)
